...
class HrGratuity(models.Model):
    """Manages employee gratuity calculations and processes. It handles
        gratuity based on contract type, service duration, wage type, and
        probation period. The module also generates accounting entries
        upon approval."""
    _name = 'hr.gratuity'
    ...
    state = fields.Selection([
        ...
        ('cancel', 'Cancelled')], default='draft', tracking=True, string="State",
        help="State of the gratuity.")
    name = fields.Char(
        string='Reference', required=True, copy=False, readonly=True,
        default=lambda self: _('New'), help="Name of the gratuity.")
    employee_id = fields.Many2one(
        'hr.employee', string='Employee',
        required=True, help="Employee who has the gratuity.")
    employee_contract_type = fields.Selection([
        ('limited', 'Limited'), ('unlimited', 'Unlimited')], readonly=True,
        string='Contract Type', store=True,
        help="Choose the contract type.if contract type is limited then during "
             "gratuity settlement if you have not specify the end date for "
             "contract, gratuity configration of limited type will be taken or"
             " if contract type is Unlimited then during gratuity settlement "
             "if you have specify the end date for contract, gratuity "
             "configration of limited type will be taken.")
    ...
    total_working_years = fields.Float(
        string='Total Years Worked', readonly=True, store=True,
    ...
    employee_gratuity_duration_id = fields.Many2one(
        'gratuity.configuration', readonly=True,
        string='Configuration Line', help="Employee gratuity duration")
    employee_gratuity_configuration_id = fields.Many2one(
        'hr.gratuity.accounting.configuration', readonly=True,
        string='Gratuity Configuration', help="Configuration for the Gratuity")
    employee_gratuity_amount = fields.Float(
        string='Gratuity Payment', readonly=True, store=True,
        help="Gratuity amount for the employee. it is calculated If the wage "
             "type is hourly then gratuity payment is calculated as employee "
             "basic salary * Employee Daily Wage Days * gratuity configration "
             "rule percentage * gratuity calculation years.orIf the wage type "
             "is monthly then gratuity payment is calculated as employee basic "
             "salary * (Working Days/Employee Daily Wage Days) * gratuity "
             "configration rule percentage * gratuity calculation years")
    hr_gratuity_credit_account_id = fields.Many2one(
        'account.account', string='Credit Account',
        help="Credit Account of the Gratuity.")
    hr_gratuity_debit_account_id = fields.Many2one(
        'account.account', string='Debit Account',
        help="Debit Account of the Gratuity.")
    hr_gratuity_journal_id = fields.Many2one(
        'account.journal',string='Gratuity Journal',
        help="Journal of the Gratuity")
    company_id = fields.Many2one(
        'res.company', 'Company', required=True,
        default=lambda self: self.env.company,
                                 help="Company of the gratuity.")
    currency_id = fields.Many2one(
        related="company_id.currency_id", string="Currency", readonly=True,
        help="Currency of the gratuity.")
